...
function formatFloat(value,decimals){
  ...
  let tmp = Math.round((value*Math.pow(10,decimals))/Math.pow(10,decimals));
  let result = tmp.toString();
  let index = result.indexOf(".");
  if(index<0 && decimals >0){
    result += ".";
  };
  for(let i=result.length - index;i<decimals;i++){
    result += "0";
  };
 ...
 */
function formatTableData(formatItem,data,decimals){
  //遍历表格数据
  for(let d of data){
    for(let val of formatItem){
      d[val.prop] = formatFloat(d[val.prop],decimals);
    }
  }
  return data;
}
